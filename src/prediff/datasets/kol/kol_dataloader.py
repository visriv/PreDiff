"""Code is adapted from https://github.com/amazon-science/earth-forecasting-transformer/blob/e60ff41c7ad806277edc2a14a7a9f45585997bd7/src/earthformer/datasets/kol/kol_dataloader.py."""
import os
...
from ...utils.path import default_dataset_kol_dir


# kol Dataset constants
kol_DATA_TYPES = ['vis', 'ir069', 'ir107', 'vil', 'lght']
kol_RAW_DTYPES = {'vis': np.int16,
                    'ir069': np.int16,
                    'ir107': np.int16,
                    'vil': np.uint8,
                    'lght': np.int16}
LIGHTING_FRAME_TIMES = np.arange(- 120.0, 125.0, 5) * 60
kol_DATA_SHAPE = {'lght': (48, 48), }
PREPROCESS_SCALE_kol = {'vis': 1,  # Not utilized in original paper
                          'ir069': 1 / 1174.68,
                          'ir107': 1 / 2562.43,
                          'vil': 1 / 47.54,
                          'lght': 1 / 0.60517}
PREPROCESS_OFFSET_kol = {'vis': 0,  # Not utilized in original paper
                           'ir069': 3683.58,
                           'ir107': 1552.80,
                           'vil': - 33.44,
                           'lght': - 0.02990}
...
class kolDataLoader:
    r"""
    DataLoader that loads kolmogorov sequences, and spilts each event
    ...
    def __init__(self,
                 ...
                 kol_catalog: Union[str, pd.DataFrame] = None,
                 kol_data_dir: str = None,
        ...
        kol_catalog
            Name of kol catalog CSV file.
        kol_data_dir
            Directory path to kol data.
        start_date
            Start time of kol samples to generate.
        end_date
            End time of kol samples to generate.
        datetime_filter
            function
        ...
        """
        super(kolDataLoader, self).__init__()
        if kol_catalog is None:
            kol_catalog = kol_CATALOG
        if kol_data_dir is None:
            kol_data_dir = kol_DATA_DIR
        if data_types is None:
            data_types = kol_DATA_TYPES
        else:
            assert set(data_types).issubset(kol_DATA_TYPES)

        # configs which should not be modified
        self._dtypes = kol_RAW_DTYPES
        self.lght_frame_times = LIGHTING_FRAME_TIMES
        self.data_shape = kol_DATA_SHAPE
        ...
        if isinstance(kol_catalog, str):
            self.catalog = pd.read_csv(kol_catalog, parse_dates=['time_utc'], low_memory=False)
        else:
            self.catalog = kol_catalog
        self.kol_data_dir = kol_data_dir
    ...
    def _compute_samples(self):
        ...
        # If there are repeated IDs, remove them (this is a bug in kol)
    ...
    def _open_files(self, verbose=True):
        ...
        for f in hdf_filenames:
            ...
            self._hdf_files[f] = h5py.File(self.kol_data_dir + '/' + f, 'r')
    ...
    def preprocess_data_dict(data_dict, data_types=None, layout='NHWT', rescale='01'):
        ...
        rescale:    str
            'kol': use the offsets and scale factors in original implementation.
        ...
        """
        if rescale == 'kol':
            scale_dict = PREPROCESS_SCALE_kol
            offset_dict = PREPROCESS_OFFSET_kol
        elif rescale == '01':
    ...
    def process_data_dict_back(data_dict, data_types=None, rescale='01'):
        ...
        rescale
            str:
                'kol': data are scaled using the offsets and scale factors in original implementation.
    ...
    def downsample_data_dict(data_dict, data_types=None, factors_dict=None, layout='NHWT'):
        ...
        if data_types is None:
            data_types = data_dict.keys()
        downsampled_data_dict = kolDataLoader.data_dict_to_tensor(
            data_dict=data_dict,

...
def vis_kol_custom(
        ...
        label: Union[str, Sequence[str]] = "pred",
        nrows: int = 2,
        ncols: Sequence[int] = [10,10],
        maxcols: int = 10,
        ):


    if (list(seq[0].shape)[-1] == 1):
        data_array = [np.squeeze(seq[i], axis = len(seq[i].shape)-1) for i in range(len(seq))]
    else:
        data_array = [seq[i] for i in range(len(seq))]
        
    flat_data = [item for sublist in data_array for item in sublist]
    ...
    # Coordinates and extents for each subplot
    # coords = [(0, 0, 90, 180), (0, 1, 90, -180), (1, 0, -90, -180), (1, 1, -90, -180)]

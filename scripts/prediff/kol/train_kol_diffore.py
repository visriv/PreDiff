...
from prediff.datasets.kol.evaluation import kolSkillScore
...
# from prediff.diffusion.knowledge_alignment.sevir import kolAvgIntensityAlignment


pytorch_state_dict_name = "kol_earthformerunet.pt"
...
class DifforekolPLModule(LatentDiffusion):
    ...
    def __init__(self,
        ...
        super(DifforekolPLModule, self).__init__(
        ...
        if self.use_alignment:
            ...
            self.alignment_obj = kolAvgIntensityAlignment(
        ...
        if self.oc.eval.eval_unaligned:
            ...
            self.valid_score = kolSkillScore(
        ...
        if self.oc.eval.eval_aligned:
            ...
            self.valid_aligned_score = kolSkillScore(
                mode=self.oc.dataset.metrics_mode,
                seq_len=self.oc.layout.out_len,
                layout=self.layout,
                threshold_list=self.oc.dataset.threshold_list,
            ...
            self.test_aligned_score = kolSkillScore(
                mode=self.oc.dataset.metrics_mode,
                seq_len=self.oc.layout.out_len,
                layout=self.layout,
                threshold_list=self.oc.dataset.threshold_list,
    ...
    @staticmethod
    def get_kol_datamodule(dataset_cfg,
    ...
    def get_input(self, batch, **kwargs):
        ...
        """
        return self._get_input_kol(batch=batch, return_verbose=kwargs.get("return_verbose", False))

    @torch.no_grad()
    def _get_input_kol(self, batch, return_verbose=False):
        seq = batch
    ...
    def save_vis_step_end(
        ...
        if data_idx in example_data_idx_list:
            ...
            vis_kol_custom(save_path=os.path.join(self.example_save_dir, png_save_name),
                    seq=seq_list,
                    label=label_list,
                    nrows = 3,
...
def get_parser():
    ...
    parser.add_argument('--save', default='tmp_kol', type=str)
    ...
    parser.add_argument('--ckpt_name', default=None, type=str,
                        help='The model checkpoint trained on kol.')
...
def main():
    ...
    if args.pretrained:
        args.cfg = os.path.abspath(os.path.join(os.path.dirname(__file__), "diffore_kol_v1.yaml"))
        # Download pretrained weights
        download_pretrained_weights(ckpt_name=pretrained_kol_vae_name,
    ...
    if args.cfg is not None:
        ...
        dataset_cfg = OmegaConf.to_object(DifforekolPLModule.get_dataset_config())
        micro_batch_size = 1
